import { Simple, ISimplifiable } from "@asmartbear/simplified";
export type Primative = boolean | number | string | null;
export type JSONType = null | boolean | string | number | JSONType[] | {
    [K: string]: JSONType;
} | {
    [K: number]: JSONType;
};
 ...
 */
export declare function isPrimative(x: unknown): x is Primative;
/**
 * The exception thrown when `input()` goes wrong.  Includes the fundamental problem,
 * and a full path to where the issue lies.
 */
export declare class ValidationError extends Error implements ISimplifiable<string> {
    path: string[];
    private readonly myMessage;
    constructor(type: IDescriptive, valueEncountered: any, expectedPrefix?: string, path?: string[]);
    addPath(segment: string | number | symbol): void;
 ...
 * as arrays of pairs.
 */
export declare abstract class SmartTypeVisitor<T, STRING extends T = T> {
    ...
    visitTuple(x: T[]): T;
    visitDate(x: Date): T;
    visitRegExp(x: RegExp): T;
    visitSet(x: T[]): T;
    visitMap(x: [T, T][]): T;
    visitFields(x: [STRING, T][]): T;
}
/** Symbol used for the configured default value for the type, if any. */
export declare const __DEFAULT_VALUE: unique symbol;
/**
 ...
 */
export declare abstract class SmartType<T = any, J extends JSONType = JSONType> implements INativeParser<T>, IMarshallJson<T, J>, ISimplifiable<Simple> {
    description: string;
    /** If not `undefined`, a default value to use in things like structured fields, if it is missing. */
    [__DEFAULT_VALUE]: T | undefined;
 ...
 */
export type NativeFor<ST> = ST extends SmartType<undefined, any> ? undefined : ST extends SmartType<null, any> ? null : ST extends SmartType<infer T, any> ? T : ST extends SmartType[] ? NativeFor<ValuesOf<ST>> : ST extends {
    readonly [K: string]: SmartType;
} ? ({
 ...
 */
export type JsonFor<ST> = ST extends SmartType<any, infer J> ? J : ST extends SmartType[] ? JsonFor<ValuesOf<ST>> : ST extends {
    readonly [K: string]: SmartType;
} ? ({
